...
<body>
    ...
    if ($stmt === false) {
        die("Error preparing the statement: " . $database->error);
    }
    ...
    if ($result && $result->num_rows > 0) {
        $userfetch = $result->fetch_assoc(); // Fetch the row as an associative array
        $userid = $userfetch["pid"]; // Assuming "pid" is the column for patient ID
        $username = $userfetch["pname"]; // Assuming "pname" is the column for patient name
    } else {
